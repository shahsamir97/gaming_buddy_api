    ...
    con.query(sql, [email], function (err, result){
        if (err) {
            ...
            if(result.length < 1) {
                ...
                var sql = "INSERT INTO user ( name, email, password, age, institution, country, city) VALUES (?,?,?,?,?,?,?)";
                con.query(sql, [name, email, hashedPassword, age, institution, country, city], function (err, result) {
                    if (err) {
                        //Create user failed
                        ...
                        response.status(200).send({auth: true, token: token, message: "User Created Successfully"})
                    }
                });
            }else{
                //when email exist
                response.status(409).send({auth: false, token: null, message: "Email Already Exist"})
            }
        }
    })
